  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .logo{
      ...
      display:grid;place-items:center;color:#04261d;font-weight:900;font-size:20px
    }
    ...
    .btn-primary{background:linear-gradient(135deg,var(--green),#00b386);color:#04261d}
    ...
    .watch-item,.recent-item{
      padding:12px 14px;border-radius:18px;background:rgba(7,15,23,.52);border:1px solid rgba(145,168,183,.1);min-width:132px
    }
    .watch-item .sym,.recent-item .sym{font-weight:800;font-size:.9rem}
    .watch-item .price{font-size:1rem;font-weight:800;margin-top:8px}
    ...
    .range-btn{
      background:rgba(17,31,44,.88);border:1px solid rgba(145,168,183,.12);color:#cfe3f0;
      padding:8px 12px;border-radius:999px;cursor:pointer;font-size:.82rem
    }
    .range-btn.active{background:rgba(0,208,156,.14);border-color:rgba(0,208,156,.35);color:#eafff9}
...
<body>
    ...
    <div class="topbar">
      <div class="brand">
        ...
        <div>
          ...
          <div class="subtitle">Live price tracking, intraday charting, and risk analytics in one screen.</div>
        </div>
      </div>
    ...
    <div class="hero">
      <div class="panel search-panel">
        <div class="search-copy">
          ...
          <h1>Track live stock price, market mood, and model-based signals in one place.</h1>
          <div class="lead">Enter any supported ticker like <b>AAPL</b>, <b>MSFT</b>, <b>NVDA</b>, or Indian symbols like <b>RELIANCE.NS</b> and the app will load a live quote, fast charts, and the full analytics pipeline.</div>
    ...
    <div class="grid">
      <div class="stack">
        <div class="panel chart-card">
          <div class="card-head">
            <div>
              <div class="card-title">Live intraday chart</div>
              <div class="card-sub">Fast price history pulled from the backend quote service.</div>
            </div>
      ...
      <div class="stack">
        <div class="panel card">
          ...
          <div class="overview">
            <div class="stat"><div class="k">Confidence</div><div class="v" id="confidenceValue">--</div></div>
            <div class="stat"><div class="k">Stress regime</div><div class="v" id="regimeValue">--</div></div>
            <div class="stat"><div class="k">Bull score</div><div class="v" id="bullValue">--</div></div>
            <div class="stat"><div class="k">Bear score</div><div class="v" id="bearValue">--</div></div>
        ...
        <div class="panel card">
          ...
          <div class="note">
            <p>Live quote API with auto-refresh, chart history API for intraday and longer ranges, in-memory recent search tracking, watchlist endpoint, and short-term caching for faster repeat requests are now built into the app.</p>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    async function loadHistory(){
      try{
        const res = await fetch(`/api/history?ticker=${encodeURIComponent(state.ticker)}&range=${encodeURIComponent(state.range)}`);
        const d = await res.json();
        if(!res.ok) throw new Error(d.error || 'Unable to load chart history');
        const y = d.points.map(p => p.close);
        const x = d.points.map(p => new Date(p.time));
        const pos = y.length && y[y.length-1] >= y[0];
        Plotly.newPlot('intradayChart', [{
          x,
          y,
          type:'scatter',
          mode:'lines',
          line:{color:pos ? COLORS.green : COLORS.red,width:2.3},
          fill:'tozeroy',
          fillcolor:pos ? 'rgba(0,208,156,.10)' : 'rgba(255,107,107,.10)'
        }], {
          ...chartLayout,
          margin:{t:10,r:10,b:34,l:54},
          yaxis:{gridcolor:COLORS.grid, zerolinecolor:COLORS.grid},
          xaxis:{gridcolor:COLORS.grid, zerolinecolor:COLORS.grid}
        }, chartConfig);
      }catch(err){
    ...
    async function analyze(){
      try{
        ...
        renderAnalysis(d);
        await Promise.all([loadHistory(), loadWatchlist(), loadRecent()]);
